...
namespace Fudp
{
    public static class DeviceLocator
    {
        ...
        {
            using (var buffer = new CanFramesBuffer(OnPort, CanProg.FuDev))
            {
                if (Template.BlockSerialNumber != 0) Template.BlockSerialNumber = 0;

                var HelloMessage = new Messages.ProgInit(Template);
                IsoTp.BeginSend(OnPort, CanProg.FuProg, CanProg.FuDev, HelloMessage.Encode()).Wait();

                return
                    buffer.Read(TimeSpan.FromMilliseconds(EchoTimeout), false)
